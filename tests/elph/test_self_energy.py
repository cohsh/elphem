...
class TestUnit(TestCase):
    ...
    def test_coupling(self):
        couplings_fan = self.electron_phonon.couplings_fan
        
        self.assertEqual(couplings_fan.shape, (self.electron_phonon.n_bands, self.electron.n_bands, self.electron.n_k, self.phonon.n_q))
    
    def test_self_energies_single(self):
        self_energies = self.electron_phonon.calculate_self_energies_fan(0.0)
        
        self.assertEqual(self_energies.shape, (self.electron_phonon.n_bands, self.electron.n_k))
    
    def test_self_energies_array(self):
        omega_array = np.linspace(-1.0, 1.0, 50)
        self_energies = self.electron_phonon.calculate_self_energies_fan(omega_array)
        
        self.assertEqual(self_energies.shape, (self.electron_phonon.n_bands, self.electron.n_k, len(omega_array)))
    
    def test_spectrum(self):
        omega_array = np.linspace(-1.0, 1.0, 50)
        self_energies = self.electron_phonon.calculate_spectrum(omega_array)

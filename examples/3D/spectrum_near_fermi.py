...
def main():
    ...
    electron_phonon = ElectronPhonon(electron, phonon, temperature, sigma=0.001, eta=0.0005)

    range_omega = [-2.0 * Energy.EV["->"], 0.0 * Energy.EV["->"]]
    ...
    ax.plot(k_path.minor_scales, electron_phonon.eigenenergies[0] * Energy.EV['<-'], linestyle='dashed', color='tab:gray')
        
    ax.set_ylim([range_omega[0] * Energy.EV['<-'], range_omega[1] * Energy.EV['<-']])

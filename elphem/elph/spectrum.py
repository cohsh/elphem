...
class Spectrum:
    ...
    def get_with_grid(self, n_k: np.ndarray, n_omega: int, range_omega: list[float]) -> np.ndarray:
        ...
        count = 0
        for omega in omegas:
            self_energy = np.array([self.electron_phonon.get_self_energy(omega, g_i, k_i) for g_i, k_i in zip(g, k)]).reshape(shape_mesh)
        
            numerator = - self_energy.imag / np.pi
            denominator = (omega - eig_grid - self_energy.real) ** 2 + self_energy.imag ** 2
